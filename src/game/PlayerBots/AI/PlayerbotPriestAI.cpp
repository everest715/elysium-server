...
CombatManeuverReturns PlayerbotPriestAI::DoFirstCombatManeuverPVE(Unit* /*pTarget*/)
{
	...
	if((spec == PRIEST_SPEC_HOLY || spec == PRIEST_SPEC_DISCIPLINE) && (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_NODISPEL) == 0)
...
CombatManeuverReturns PlayerbotPriestAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    if (FEAR_WARD > 0 && m_bot->IsSpellReady(FEAR_WARD) && SHADOWFORM == 0)
    {
        // Buff tank first
        if (pMainTank)
        {
    ...
    if (newTarget && !m_ai->IsNeutralized(newTarget) && m_ai->IsElite(newTarget)) // TODO: && party has a tank
    {
        if (FADE > 0 && !m_bot->HasAura(FADE, EFFECT_INDEX_0) && m_bot->IsSpellReady(FADE))
        {
            if (CastSpell(FADE, m_bot))
            {
                m_ai->TellMaster(" 我要挨揍了，不能再浪了~~ ") ;
                return RETURN_CONTINUE;
            }
            else
                m_ai->TellMaster(" 我被怪盯上了~~ ");
        }

        // Heal myself
        // TODO: move to HealTarget code
		Creature * pCreature = dynamic_cast<Creature*>(newTarget);
		if (pCreature && (pCreature->GetCreatureInfo()->type == CREATURE_TYPE_UNDEAD) && m_ai->GetFightFlg())
		{
			if (SHACKLE_UNDEAD && CastSpell(SHACKLE_UNDEAD, newTarget))
				return RETURN_NO_ACTION_OK;
		}
        if (m_ai->GetHealthPercent() < 35 && POWER_WORD_SHIELD > 0 && !m_bot->HasAura(POWER_WORD_SHIELD, EFFECT_INDEX_0) && !m_bot->HasAura(WEAKNED_SOUL, EFFECT_INDEX_0))
        {
            ...
            {
                m_ai->TellMaster(" 给自己上个蛋壳，也没人管我，唉~~ ") ;
                return RETURN_CONTINUE;
            }
            else if (m_ai->IsHealer()) // Even if any other RETURN_ANY_OK - aside from RETURN_CONTINUE
                m_ai->TellMaster(" 主人救救我啊，你的小奶妈要被打死了~~ ") ;
        }
        if (m_ai->GetHealthPercent() < 20 && DESPERATE_PRAYER > 0 && m_ai->In_Reach(m_bot,DESPERATE_PRAYER) && CastSpell(DESPERATE_PRAYER, m_bot) & RETURN_CONTINUE)
        {
            m_ai->TellMaster(" 瞬间加血，牧师也会~~ ") ;
            return RETURN_CONTINUE;
        }
    ...
    if (m_ai->IsHealer())
    {
		...
		if (m_bot->GetGroup())
		{
			...
			for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++)
			{
				...
				{
					if (RENEW && m_ai->In_Reach(pet, RENEW) && !pet->HasAura(RENEW) && m_ai->CastSpell(RENEW, *pet))
						return RETURN_CONTINUE;
				}
			}
		}
    ...
    else
    {
        ...
        if ((spec == 0 || (spec == PRIEST_SPEC_SHADOW  && SHADOWFORM == 0)) && !inRaidGroup)
        {
    ...
    switch (spec)
    {
        ...
        case PRIEST_SPEC_SHADOW:
            if (SHADOWFORM > 0 && !m_bot->HasAura(SHADOWFORM, EFFECT_INDEX_0))
            ...
            if (DEVOURING_PLAGUE > 0 && m_ai->In_Reach(pTarget,DEVOURING_PLAGUE) && !pTarget->HasAura(DEVOURING_PLAGUE, EFFECT_INDEX_0) && CastSpell(DEVOURING_PLAGUE, pTarget))
                return RETURN_CONTINUE;
            if (VAMPIRIC_EMBRACE > 0 && m_ai->In_Reach(pTarget,VAMPIRIC_EMBRACE) && !pTarget->HasAura(VAMPIRIC_EMBRACE, EFFECT_INDEX_0) && CastSpell(VAMPIRIC_EMBRACE, pTarget))
                return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPriestAI::HealPlayer(Player* target)
{
    ...
    if (hp < 70 && HEAL > 0 && m_ai->In_Reach(target,HEAL) && m_ai->CastSpell(HEAL, *target))
        return RETURN_CONTINUE;
    if (hp < 85 && RENEW > 0 && m_ai->In_Reach(target,RENEW) && !target->HasAura(RENEW) && m_ai->CastSpell(RENEW, *target))
        return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPriestAI::Healothers(Player* target)
{
    CombatManeuverReturns r = PlayerbotClassAI::HealPlayer(target);
    if (r != RETURN_NO_ACTION_OK)
        return r;
    ...
    // Get a free and more efficient heal if needed: low mana for bot or average health for target
    if (hp < 20 && POWER_WORD_SHIELD > 0 && m_ai->In_Reach(target, POWER_WORD_SHIELD) && !m_bot->HasAura(POWER_WORD_SHIELD, EFFECT_INDEX_0) && !target->HasAura(WEAKNED_SOUL, EFFECT_INDEX_0) && m_ai->CastSpell(POWER_WORD_SHIELD, *target))
        return RETURN_CONTINUE;
    if (hp < 40 && HEAL > 0 && m_ai->In_Reach(target, HEAL) && m_ai->CastSpell(HEAL, *target))
        return RETURN_CONTINUE;
    if (hp < 60 && RENEW > 0 && m_ai->In_Reach(target, RENEW) && !target->HasAura(RENEW) && m_ai->CastSpell(RENEW, *target))
        return RETURN_CONTINUE;
...
void PlayerbotPriestAI::DoNonCombatActions()
{
    ...
    if (m_ai->GetFightFlg())
    {
        ...
        if (NeedGroupBuff(PRAYER_OF_SPIRIT, DIVINE_SPIRIT) && m_ai->HasSpellReagents(PRAYER_OF_SPIRIT))
        {
            if (Buff(&PlayerbotPriestAI::BuffHelper, PRAYER_OF_SPIRIT) & RETURN_CONTINUE)
                return;
        }
        else if (Buff(&PlayerbotPriestAI::BuffHelper, DIVINE_SPIRIT) & RETURN_CONTINUE)
            return;

        if (NeedGroupBuff(PRAYER_OF_SHADOW_PROTECTION, SHADOW_PROTECTION) && m_ai->HasSpellReagents(PRAYER_OF_SHADOW_PROTECTION))
        {

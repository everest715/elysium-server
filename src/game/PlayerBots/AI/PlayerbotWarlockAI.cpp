...
PlayerbotWarlockAI::PlayerbotWarlockAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
	...
	Summon_Dreadsteed     = m_ai->initSpell(Summon_Dreadsteed_1);
	Summon_Felsteed       = m_ai->initSpell(Summon_Felsteed_1);
...
CombatManeuverReturns PlayerbotWarlockAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    if (newTarget && !m_ai->IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        {
            // If elite
            if (m_ai->IsElite(newTarget) && m_ai->GetFightFlg() && pTarget != newTarget)
            {
                ...
                if (pet)
                {
                    switch (pet->GetEntry())
                    {
                        ...
                        case DEMON_SUCCUBUS:
                            if (pCreature && pCreature->GetCreatureInfo()->type == CREATURE_TYPE_HUMANOID)
                ...
                if (pCreature && (pCreature->GetCreatureInfo()->type == CREATURE_TYPE_DEMON || pCreature->GetCreatureInfo()->type == CREATURE_TYPE_ELEMENTAL))
                {
	...
	ulBANISHtimes = 0;//·ÅÖð¼ÆÊý£¬·ÀÖ¹ÎÞÏÞ·ÅÖð

	if (pTarget->GetEntry() == 14862 || pTarget->GetEntry() == 15340)
	{
		if (DRAIN_MANA > 0 && m_ai->In_Reach(pTarget, DRAIN_MANA) && m_ai->CastSpell(DRAIN_MANA, *pTarget));
    ...
    uint8 HPThreshold = (m_ai->IsElite(pTarget) ? 10 : 80);
	if (m_bot->getLevel() == 60)
	{
		m_MAX_SHARD_COUNT = 56;
	}
	else
	{
		m_MAX_SHARD_COUNT = 16;
	}

    if (!m_ai->IsWorldBoss(pTarget) && pTarget->GetHealthPercent() < HPThreshold && (shardCount < m_MAX_SHARD_COUNT && freeSpace > 0))
    {
		if (shardCount > 50 && m_ai->IsElite(pTarget))
		{
    ...
    if (m_ai->GetFightFlg() && m_ai->GetAttackerCount() >= 4 /*&& ((!pTarget->GetEntry() == 8570) || (!pTarget->GetEntry() == 11121) || (!pTarget->GetEntry() == 1204))*/)
	{
	    if (RAIN_OF_FIRE > 0 && m_ai->In_Reach(pTarget, RAIN_OF_FIRE) && CastSpell(RAIN_OF_FIRE, pTarget))
		{
    ...
    if (spec)
    {
        switch (spec)
        {
            case WARLOCK_SPEC_AFFLICTION:
				...
				//Ï×¼À 348
				if (IMMOLATE && m_ai->In_Reach(pTarget, IMMOLATE) && !pTarget->HasAura(IMMOLATE) && !inRaidGroup && CastSpell(IMMOLATE, pTarget) && (!m_bot->GetMapId() == 409))
				{
					return RETURN_CONTINUE;
				}
                break;
            ...
            case WARLOCK_SPEC_DESTRUCTION:
                ...
                /*if (SHADOWBURN && pTarget->GetHealthPercent() < (HPThreshold / 2.0) && m_ai->In_Reach(pTarget, SHADOWBURN) && !pTarget->HasAura(SHADOWBURN) && CastSpell(SHADOWBURN, pTarget))
                    return RETURN_CONTINUE;*/
				//Ï×¼À 348
				if (IMMOLATE && m_ai->In_Reach(pTarget, IMMOLATE) && !pTarget->HasAura(IMMOLATE) && CastSpell(IMMOLATE, pTarget) && (!m_bot->GetMapId() == 409))
				{
					return RETURN_CONTINUE;
				}
				//¸¯Ê´Êõ 172
				if (CORRUPTION && m_ai->In_Reach(pTarget, CORRUPTION) && !pTarget->HasAura(CORRUPTION) && !inRaidGroup && CastSpell(CORRUPTION, pTarget))
				{
					return RETURN_CONTINUE;
				}
				//È¼ÉÕ 17962
...
bool PlayerbotWarlockAI::CheckCurse(Unit* pTarget)
{
    ...
    if (m_bot->GetGroup())
    {
        if (pCreature && m_bot->GetGroup()->isRaidGroup())
        {
            uint8 mages = 0;
            uint8 warlocks = 0;
            ...
            else if (warlocks > 1)
            {
                ...
                {
                    if (AMPLIFY_CURSE && m_ai->In_Reach(pTarget, CURSE_OF_AGONY) && !pTarget->HasAura(CURSE_OF_AGONY))
                    {
                        CurseToCast = CURSE_OF_AGONY;
                    }
                    else if (!AMPLIFY_CURSE && m_ai->In_Reach(pTarget, CURSE_OF_SHADOW) && !pTarget->HasAura(CURSE_OF_SHADOW))
                    {
                        CurseToCast = CURSE_OF_SHADOW;
                    }
                }
                if (spec == WARLOCK_SPEC_DEMONOLOGY)
                {
                    CurseToCast = CURSE_OF_SHADOW;
                }
                if (spec == WARLOCK_SPEC_DESTRUCTION)
                {
                    CurseToCast = CURSE_OF_THE_ELEMENTS;
                }
            }
        ...
        else if (!m_bot->GetGroup()->isRaidGroup())
            CurseToCast = CURSE_OF_AGONY;
    }
...
void PlayerbotWarlockAI::CheckDemon()
{
    ...
    if (spec == WARLOCK_SPEC_AFFLICTION)
        ...
        demonOfChoice = (DEMON_SUCCUBUS > 0 ? DEMON_SUCCUBUS : DEMON_VOIDWALKER);
...
void PlayerbotWarlockAI::DoNonCombatActions()
{
    ...
    uint8 shardCount = m_bot->GetItemCount(SOUL_SHARD, false, nullptr);
    uint8 freeSpace = m_ai->GetFreeBagSpace();
    /*if (shardCount > m_MAX_SHARD_COUNT || (freeSpace == 0 && shardCount > 1))
        m_bot->DestroyItemCount(SOUL_SHARD, shardCount > m_MAX_SHARD_COUNT ? shardCount - m_MAX_SHARD_COUNT : 1, true, false);*/
	...
	if (master && master->HasAuraType(SPELL_AURA_MOUNTED))
	{
		...
		else if (Summon_Dreadsteed > 0)
		{
			m_ai->CastSpell(Summon_Dreadsteed);
		}
		else if (Summon_Felsteed > 0)
		{
			m_ai->CastSpell(Summon_Felsteed);
		}
	}

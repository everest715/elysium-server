...
PlayerbotRogueAI::PlayerbotRogueAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
	...
	BLADE_FLURRY             = m_ai->initSpell(BLADE_FLURRY_1);
...
CombatManeuverReturns PlayerbotRogueAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    if (COLD_BLOOD > 0 && !m_bot->HasAura(COLD_BLOOD, EFFECT_INDEX_0) && m_bot->IsSpellReady(COLD_BLOOD) && m_ai->CastSpell(COLD_BLOOD, *m_bot))
            return RETURN_CONTINUE;
	if (BLADE_FLURRY > 0 && !m_bot->HasAura(BLADE_FLURRY, EFFECT_INDEX_0) && m_bot->IsSpellReady(BLADE_FLURRY) && m_ai->CastSpell(BLADE_FLURRY, *m_bot))
		    return RETURN_CONTINUE;
    ...
    if (m_ai->GetEnergyAmount() < 20 && ADRENALINE_RUSH > 0 &&(m_ai->IsElite(pTarget) || m_ai->IsWorldBoss(pTarget))/*&& pTarget->GetHealthPercent() < 50 */&& !m_bot->HasAura(ADRENALINE_RUSH, EFFECT_INDEX_0) && m_bot->IsSpellReady(ADRENALINE_RUSH) && m_ai->CastSpell(ADRENALINE_RUSH, *m_bot))
        return RETURN_CONTINUE;
    ...
    if (m_bot->GetComboPoints() >= 4)
    {
        ...
        if (SLICE_DICE > 0 && (m_ai->IsElite(pTarget) || m_ai->IsWorldBoss(pTarget)) && !m_bot->HasAura(SLICE_DICE, EFFECT_INDEX_1) && m_ai->CastSpell(SLICE_DICE, *pTarget)) // 25 energy (checked above)
            return RETURN_CONTINUE;
        ...
        if (HEMORRHAGE > 0 && RUPTURE > 0 && !pTarget->HasAura(RUPTURE, EFFECT_INDEX_0) && m_ai->CastSpell(RUPTURE, *pTarget)) // 25 energy (checked above)
            return RETURN_CONTINUE;

        // default combo action or if other combo action is unavailable/failed
        // wait for energy
        if (m_ai->GetEnergyAmount() > 35 && EVISCERATE > 0 && m_ai->CastSpell(EVISCERATE, *pTarget))
            return RETURN_CONTINUE;
	...
	if (weapon && weapon->GetSpell() == 1180 && pTarget->isInBackInMap(m_bot, 5.0f))
	{
		if (BACKSTAB > 0 && m_ai->CastSpell(BACKSTAB, *pTarget))
			return RETURN_CONTINUE;	
	}
	else
	{

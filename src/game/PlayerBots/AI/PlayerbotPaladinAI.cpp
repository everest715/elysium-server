...
PlayerbotPaladinAI::PlayerbotPaladinAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
	...
	Summon_Charger                = m_ai->initSpell(Summon_Charger_1);
	Summon_Warhorse               = m_ai->initSpell(Summon_Warhorse_1);
...
CombatManeuverReturns PlayerbotPaladinAI::DoFirstCombatManeuverPVE(Unit* /*pTarget*/)
{
	...
	if (spec == PALADIN_SPEC_HOLY && (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_NODISPEL) == 0)
...
CombatManeuverReturns PlayerbotPaladinAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    if (newTarget && !(m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_TANK) && !m_ai->IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        else 
        {
            ...
            if ((spec == 0 || spec == PALADIN_SPEC_RETRIBUTION) && !inRaidGroup)
            {
    ...
    if (m_ai->IsHealer())
    {
        ...
        if (m_bot->GetGroup())
        {
            ...
            for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++)
            {
                ...
                {
                    if (FLASH_OF_LIGHT && m_ai->In_Reach(pet, FLASH_OF_LIGHT) && m_ai->CastSpell(FLASH_OF_LIGHT, *pet))
                        return RETURN_CONTINUE;
                }
            }
        }
...
CombatManeuverReturns PlayerbotPaladinAI::HealPlayer(Player* target)
{
    ...
    /*if (hp < 60 && HOLY_SHOCK && m_ai->In_Reach(target,HOLY_SHOCK) && m_ai->CastSpell(HOLY_SHOCK, *target))
        return RETURN_CONTINUE;*/
    if (hp < 60 && HOLY_LIGHT && m_ai->In_Reach(target,HOLY_LIGHT) && m_ai->CastSpell(HOLY_LIGHT, *target))
        return RETURN_CONTINUE;
	
	/*if (hp < 65 && FLASH_OF_LIGHT && m_ai->In_Reach(target,FLASH_OF_LIGHT) && m_ai->CastSpell(19942, *target))
        return RETURN_CONTINUE;
	
	if (hp < 78 && FLASH_OF_LIGHT && m_ai->In_Reach(target,FLASH_OF_LIGHT) && m_ai->CastSpell(19940, *target))
        return RETURN_CONTINUE;
	
	if (hp < 90 && FLASH_OF_LIGHT && m_ai->In_Reach(target,FLASH_OF_LIGHT) && m_ai->CastSpell(19750, *target))
        return RETURN_CONTINUE;*/
	if (hp < 85 && FLASH_OF_LIGHT && m_ai->In_Reach(target, FLASH_OF_LIGHT) && m_ai->CastSpell(FLASH_OF_LIGHT, *target))
		return RETURN_CONTINUE;

	return RETURN_NO_ACTION_OK;
} // end HealTarget

CombatManeuverReturns PlayerbotPaladinAI::Healothers(Player* target)
{
    CombatManeuverReturns r = PlayerbotClassAI::HealPlayer(target);
    if (r != RETURN_NO_ACTION_OK)
        return r;
    ...
    /*if (hp < 60 && HOLY_SHOCK && m_ai->In_Reach(target,HOLY_SHOCK) && m_ai->CastSpell(HOLY_SHOCK, *target))
    return RETURN_CONTINUE;*/
    if (hp < 40 && HOLY_LIGHT && m_ai->In_Reach(target, HOLY_LIGHT) && m_ai->CastSpell(HOLY_LIGHT, *target))
        return RETURN_CONTINUE;

    if (hp < 60 && FLASH_OF_LIGHT && m_ai->In_Reach(target, FLASH_OF_LIGHT) && m_ai->CastSpell(FLASH_OF_LIGHT, *target))
        return RETURN_CONTINUE;
...
bool PlayerbotPaladinAI::CheckSealAndJudgement(Unit* pTarget)
{
    ...
    if (pCreature && pCreature->GetCreatureInfo()->type == CREATURE_TYPE_HUMANOID && pTarget->GetHealthPercent() < 20 && !pCreature->IsWorldBoss())
    {
        if (!m_ai->IsHealer() && SEAL_OF_JUSTICE > 0 && !m_bot->HasAura(SEAL_OF_JUSTICE, EFFECT_INDEX_0) && m_ai->CastSpell(SEAL_OF_JUSTICE, *m_bot))
        {
    ...
    if (m_CurrentJudgement == 0 && m_CurrentSeal > 0 && m_bot->HasAura(m_CurrentSeal, EFFECT_INDEX_0))
    {
        if (JUDGEMENT > 0 && m_bot->IsSpellReady(JUDGEMENT) && m_ai->In_Reach(pTarget, JUDGEMENT) && m_ai->CastSpell(JUDGEMENT, *pTarget))
        {
            ...
            else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 21082 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_1;
			else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 20162 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_2;
			else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 20305 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_3;
			else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 20306 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_4;
			else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 20307 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_5;
			else if (m_CurrentSeal == SEAL_OF_THE_CRUSADER && SEAL_OF_THE_CRUSADER== 20308 )
                m_CurrentJudgement = JUDGEMENT_OF_THE_CRUSADER_6;
            else
                return false;
    ...
    {
        if ((spec == PALADIN_SPEC_HOLY) && meleeReach)
            m_CurrentSeal = SEAL_OF_WISDOM;
		if (spec == PALADIN_SPEC_PROTECTION || spec == PALADIN_SPEC_RETRIBUTION)
...
void PlayerbotPaladinAI::DoNonCombatActions()
{
	...
	if (master && master->HasAuraType(SPELL_AURA_MOUNTED))
	{
		...
		else if (Summon_Charger > 0)
		{
			m_ai->CastSpell(Summon_Charger);
		}
		else if (Summon_Warhorse > 0)
		{
			m_ai->CastSpell(Summon_Warhorse);
		}
	}
}
 ...
 */
bool PlayerbotPaladinAI::GetAiBuffFlag()
{
	return false;
}
...
bool PlayerbotPaladinAI::BuffHelper(PlayerbotAI* ai, uint32 spellId, Unit *target)
{
    ...
    if (pet && pet->isAlive() && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && ai->HasSpellReagents(petBigSpellId) && ai->Buff(petBigSpellId, pet))
        return true;
    if (ai->HasSpellReagents(bigSpellId) && ai->Buff(bigSpellId, target))
        return true;
    if ((pet && pet->isAlive() && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && ai->Buff(petSpellId, pet)) || ai->Buff(spellId, target))
        return true;
    return false;
}

...
PlayerbotDruidAI::PlayerbotDruidAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
    ...
    for (uint8 i = 0; i < 5; i++)
    {
        if (m_ai->initSpell(uiThickHide[i]))
            THICK_HIDE = m_ai->initSpell(uiThickHide[i]);
    }
}
...
CombatManeuverReturns PlayerbotDruidAI::DoFirstCombatManeuverPVE(Unit* /*pTarget*/)
{
	...
	if(spec == DRUID_SPEC_RESTORATION && (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_NODISPEL) == 0)
...
CombatManeuverReturns PlayerbotDruidAI::DoNextCombatManeuverPVE(Unit* pTarget)
{
    ...
    if (newTarget && !(m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_TANK) && !m_ai->IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        else 
        {
            ...
            if ((spec == 0 || (spec == DRUID_SPEC_BALANCE && !MOONKIN_FORM)) && !inRaidGroup)
            {
        ...
        if (m_ai->IsElite(newTarget))
        {
            ...
            if (m_ai->IsHealer() || spec == DRUID_SPEC_BALANCE)
            {
		    ...
		    if (pCreature && (pCreature->GetCreatureInfo()->type == CREATURE_TYPE_BEAST || pCreature->GetCreatureInfo()->type == CREATURE_TYPE_DRAGONKIN) && m_ai->GetFightFlg() && pTarget != newTarget)
		    {
			    if (HIBERNATE && CastSpell(HIBERNATE, newTarget))
                    return RETURN_NO_ACTION_OK;
            }
    ...
    if (m_ai->IsHealer())
	{
		...
		if (m_bot->GetGroup())
		{
			...
			for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++)
			{
				...
				{
					if (REJUVENATION && m_ai->In_Reach(pet, REJUVENATION) && !pet->HasAura(REJUVENATION) && m_ai->CastSpell(REJUVENATION, *pet))
						return RETURN_CONTINUE;
				}
			}
		}
    ...
    switch (spec)
    {
        case DRUID_SPEC_FERAL:
            if (BEAR > 0 && !m_bot->HasAura(BEAR) && THICK_HIDE > 0 && CastSpell(BEAR))
                return RETURN_CONTINUE;
            if (CAT_FORM > 0 && !m_bot->HasAura(CAT_FORM) && THICK_HIDE == 0 && CastSpell(CAT_FORM))
                return RETURN_CONTINUE;
        ...
        case DRUID_SPEC_BALANCE:
            if (MOONKIN_FORM > 0 && !m_bot->HasAura(MOONKIN_FORM) && CastSpell(MOONKIN_FORM))
                return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverBear(Unit* pTarget)
{
    ...
    if (m_bot->GetHealthPercent() < 70)
    {
        // Cast FRENZIED REGENERATION first because it has lower cooldown
        if (FRENZIED_REGENERATION > 0 && !m_bot->HasAura(FRENZIED_REGENERATION, EFFECT_INDEX_0) && m_bot->IsSpellReady(FRENZIED_REGENERATION) && m_ai->CastSpell(FRENZIED_REGENERATION, *m_bot))
        {
            m_ai->TellMaster("皮厚也不能不给我加血啊！ ");
            return RETURN_CONTINUE;
        }
    }
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverSpellDPS(Unit* pTarget)
{
    ...
    {
        if (HURRICANE > 0 && m_ai->In_Reach(pTarget, HURRICANE) && CastSpell(HURRICANE, pTarget))
        {
    ...
    if (FAERIE_FIRE > 0 && !m_ai->IsHealer() && m_ai->In_Reach(pTarget,FAERIE_FIRE) && !pTarget->HasAura(FAERIE_FIRE, EFFECT_INDEX_0) && CastSpell(FAERIE_FIRE, pTarget))
        return RETURN_CONTINUE;

    /*if (INSECT_SWARM > 0 && !m_ai->IsHealer() && m_ai->In_Reach(pTarget,INSECT_SWARM) && !pTarget->HasAura(INSECT_SWARM, EFFECT_INDEX_0) && CastSpell(INSECT_SWARM, pTarget))
        return RETURN_CONTINUE;*/
    ...
    if (WRATH > 0 && CastSpell(WRATH, pTarget))
        return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotDruidAI::Healothers(Player* target)
{
    CombatManeuverReturns r = PlayerbotClassAI::HealPlayer(target);
    if (r != RETURN_NO_ACTION_OK)
        return r;
    ...
    if (hp < 40 && HEALING_TOUCH > 0 && m_ai->In_Reach(target, HEALING_TOUCH) && CastSpell(HEALING_TOUCH, target))
        return RETURN_CONTINUE;

    if (hp < 60 && REJUVENATION > 0 && m_ai->In_Reach(target, REJUVENATION) && !target->HasAura(REJUVENATION) && CastSpell(REJUVENATION, target))
        return RETURN_CONTINUE;
...
bool PlayerbotDruidAI::BuffHelper(PlayerbotAI* ai, uint32 spellId, Unit* target)
{
    ...
    if (pet && pet->isAlive() && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && ai->Buff(spellId, pet, &(PlayerbotDruidAI::GoBuffForm)))
        return true;

...
PlayerbotHunterAI::PlayerbotHunterAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
	...
	PET_Growl                     = m_ai->initSpell(PET_Growl_1);
    ...
    MULTI_SHOT                    = m_ai->initSpell(MULTI_SHOT_1);
...
bool PlayerbotHunterAI::HasDeadPet(Player* bot)
{
    if (bot->GetPet() && !bot->GetPet()->isAlive())
    {
        return true;
    }

    QueryResult* result = CharacterDatabase.PQuery("SELECT curhealth FROM character_pet WHERE owner = '%u' AND slot = '%u'", bot->GetGUIDLow(), PET_SAVE_NOT_IN_SLOT);

    if (result)
    {
        Field* fields = result->Fetch();
        uint32 curhealth = fields[0].GetUInt32();
        return curhealth == 0;  //hunter has current pet
    }
    else
...
*/
    ...
    {
        m_ai->TellMaster("施放凝神射击 %s", pTarget->GetName());
        return RETURN_CONTINUE;
    }

	if (pTarget->GetEntry() == 14862 || pTarget->GetEntry() == 15340)
	{
		if (VIPER_STING > 0 && m_ai->In_Reach(pTarget, VIPER_STING) && !pTarget->HasAura(VIPER_STING, EFFECT_INDEX_0) && m_ai->CastSpell(VIPER_STING, *pTarget))
		return RETURN_CONTINUE;
	}
    ...
    if (newTarget && !m_ai->IsNeutralized(newTarget)) // TODO: && party has a tank
    {
	  // If elite
	  // Aggroed by an elite
	  if (m_ai->IsElite(newTarget) && newTarget->GetHealthPercent() > 25 && m_ai->GetFightFlg())
	  {		
        if (pTarget != newTarget)
        {
			/*if (pet)
			{
				if (PET_Growl && m_ai->CastPetSpell(PET_Growl, newTarget))
					return RETURN_NO_ACTION_OK;
                if (pet && INTIMIDATION > 0 && pVictim == pet && !pet->HasAura(INTIMIDATION, EFFECT_INDEX_0) && m_ai->CastSpell(INTIMIDATION, *m_bot))
				    return RETURN_NO_ACTION_OK;
			}    
			else
			{*/
				if (FEIGN_DEATH > 0 && m_bot->IsSpellReady(FEIGN_DEATH) && !m_bot->HasAura(FEIGN_DEATH, EFFECT_INDEX_0) && m_ai->CastSpell(FEIGN_DEATH, *m_bot))
					return RETURN_CONTINUE;
				if (FREEZING_TRAP > 0 && m_bot->IsSpellReady(FREEZING_TRAP) && m_ai->CastSpell(FREEZING_TRAP, *m_bot))
					return RETURN_NO_ACTION_OK;
				if (ASPECT_OF_THE_MONKEY > 0 && !m_bot->HasAura(ASPECT_OF_THE_MONKEY, EFFECT_INDEX_0) && (m_ai->CastSpell(ASPECT_OF_THE_MONKEY, *m_bot)))
					return RETURN_CONTINUE; 
			//}
        }
		else
	...
	if (AUTO_SHOT > 0 && m_bot->GetCombatDistance(pTarget) <= 8.0f && m_has_ammo)
	{
		if (m_bot->isAttackReady(RANGED_ATTACK))
			m_bot->CastSpell(pTarget, AUTO_SHOT, true);

		const SpellEntry* spellInfo = sSpellMgr.GetSpellEntry(AUTO_SHOT);
		if (!spellInfo)
			return RETURN_CONTINUE;
    ...
    if (m_ai->GetCombatStyle() == PlayerbotAI::COMBAT_RANGED )
    {
	  ...
	  {	 
        ...
        {*/
        if (!m_ai->IsWorldBoss(pTarget) && SERPENT_STING > 0 && m_ai->In_Range(pTarget,SERPENT_STING) && !pTarget->HasAura(SERPENT_STING, EFFECT_INDEX_0) && m_ai->CastSpell(SERPENT_STING, *pTarget))
            return RETURN_CONTINUE;
        //}
		if (pCreature && (pCreature->GetCreatureInfo()->type == CREATURE_TYPE_HUMANOID))
		{
			if (CONCUSSIVE_SHOT > 0 && !pTarget->HasAura(CONCUSSIVE_SHOT, EFFECT_INDEX_0) && pTarget->GetHealthPercent() < 10 && m_ai->CastSpell(CONCUSSIVE_SHOT, *pTarget))
				return RETURN_CONTINUE;
		}
        ...
        if (AIMED_SHOT > 0 && m_bot->IsSpellReady(AIMED_SHOT) && m_ai->In_Range(pTarget,AIMED_SHOT) && m_ai->CastSpell(AIMED_SHOT, *pTarget))
            return RETURN_CONTINUE;
        if (MULTI_SHOT > 0 && m_bot->IsSpellReady(MULTI_SHOT) && m_ai->In_Range(pTarget,MULTI_SHOT) && m_ai->CastSpell(2643, *pTarget))
            return RETURN_CONTINUE;
...
//           return RETURN_CONTINUE;
      ...
      if ( meleeReach || !m_has_ammo)
      {
        if (MONGOOSE_BITE > 0 && m_bot->RollMeleeOutcomeAgainst(pTarget, BASE_ATTACK) == MELEE_HIT_DODGE && m_ai->CastSpell(MONGOOSE_BITE, *pTarget))
            return RETURN_CONTINUE;

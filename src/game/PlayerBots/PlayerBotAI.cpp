...
m_taxiMaster(ObjectGuid())
{
	...
	m_AutolootFlg = true; //013 Bots Auto config
	m_AutoBuffFlg = true;//013 Bots Auto config
	m_AutoFightFlg = true;//013 Bots Auto config
	m_AutoDragFlg = false;//013 Bots Auto config
    m_targetType = TARGET_NORMAL;
    m_targetCombat = 0;
    m_targetAssist = 0;
    m_targetProtect = 0;
	m_AutoSurvey = false;//013 Bots Auto config
	m_BattleMapId = 0;//013 Bots Auto config
...
void PlayerbotAI::SetBattleMapId(uint32 ulMapid)
{
	m_BattleMapId = ulMapid;
}

//009 bot auto Join battle
uint32 PlayerbotAI::GetBattleMapId() const
{
	return m_BattleMapId;
}
...
uint32 PlayerbotAI::getPetSpellId(const char* args) const
{
    ...
    for (PetSpellMap::iterator itr = pet->m_petSpells.begin(); itr != pet->m_petSpells.end(); ++itr)
    {
        uint32 spellId = itr->first;
...
void PlayerbotAI::ReloadAI()
{
    switch (m_bot->getClass())
    {
        ...
        case CLASS_SHAMAN:
            if (m_classAI) delete m_classAI;
            if (m_bot->GetSpec() == SHAMAN_SPEC_ENHANCEMENT || m_bot->GetSpec() == 0)
            {
                m_combatStyle = COMBAT_MELEE;
            }
            else
...
void PlayerbotAI::SendOrders(Player& /*player*/)
{
    ...
    if (!m_combatOrder)
        ...
        out << "我会帮助他攻击的 " << (m_targetAssist ? m_targetAssist->GetName() : "unknown");
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket& packet)
{
    switch (packet.GetOpcode())
    {
        ...
        case SMSG_DUEL_REQUESTED:
        {
            ...
            {
                m_bot->GetMotionMaster()->Clear(true);
                WorldPacket packet = WorldPacket(CMSG_DUEL_ACCEPTED, 8);
                packet << flagGuid;
            ...
            if (err != EQUIP_ERR_OK)
            {
                switch (err)
                {
                    ...
                    case EQUIP_ERR_BANK_FULL:
                        TellMaster("我的银行满了,主人.");
                        return;
                    case EQUIP_ERR_ITEM_NOT_FOUND:
                        TellMaster("我找不到那东西了,主人.");
                        return;
                    case EQUIP_ERR_TOO_FAR_AWAY_FROM_BANK:
                        TellMaster("我离银行太远,主人.");
                        return;
                    default:
                        TellMaster("我不能使用它,主人.");
    ...
    case SMSG_MESSAGECHAT:
        ...
        case SMSG_GUILD_INVITE:
        {
            if (m_bot->GetGuildIdInvited())
            {
                ...
                Player* inviter = ObjectAccessor::FindPlayerByName(inviterName.c_str());
                if (!inviter)
                {
                    TellMaster("Invalide inviter!");
                    return;
                }
        ...
        case SMSG_TRADE_STATUS:
        {
            ...
            else if (status == 1)
            {
                ...
                {
                    SendWhisper("我不能给你东西但你可以给我东西或者钱嘿嘿.", *(m_bot->GetTrader()));
                    return;
                }
        ...
        case SMSG_RESURRECT_REQUEST:
        {
            if (!m_bot->isAlive())
            {
                ...
                WorldPacket packet = WorldPacket(CMSG_RESURRECT_RESPONSE, 8 + 1);
                packet << guid;
                ...
                if (c && c->GetCreatureInfo()->SkinLootId && !c->lootForSkin)
                {
                                                      ...
                                                      (HasCollectFlag(COLLECT_FLAG_SKIN) && reqSkill == SKILL_SKINNING)))
                    {
                        ...
                        if (skillValue >= reqSkillValue)
                        {
                            ...
                            TellMaster("我会 %u 但是需要 %u", skillValue, reqSkillValue);
                    }
                }
 ...
 *
 * params:weapon Item* the weap么n the function should search and return a enchanting item for
...
void PlayerbotAI::InterruptCurrentCastingSpell()
{
    ...
    WorldPacket packet = WorldPacket(CMSG_CANCEL_CAST, 5);  //changed from thetourist suggestion
...
void PlayerbotAI::Attack(Unit* forcedTarget)
{
    ...
    if (m_botState != BOTSTATE_COMBAT)
    {
        ...
        m_lootCurrent = ObjectGuid();
        m_targetCombat = 0;
        if (m_bot->GetMap()->IsDungeon())
        {
            if ((m_bot->GetSpec() == WARRIOR_SPEC_PROTECTION) || (m_bot->GetSpec() == PALADIN_SPEC_PROTECTION))
            {
                m_DelayAttack = 0;
            }
            else
...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    if (m_combatOrder & ORDERS_TANK && !(m_combatOrder & ORDERS_PROTECT) && !inRaidGroup)
    {
        if(GetGroupHealer())
        m_targetProtect = GetGroupHealer();
        else
        m_targetProtect = GetMaster();
    }
    ...
    if (!forcedTarget && (m_combatOrder & ORDERS_PROTECT) && m_targetProtect)
    {
        ...
        if (newTarget && newTarget != m_targetCombat)
        {
            ...
            //if (m_mgr->m_confDebugWhisper)
                TellMaster("%s ,放开 %s 有啥事冲我来", forcedTarget->GetName(), m_targetProtect->GetName());
        }
    }
    else if (!forcedTarget && !(m_combatOrder & ORDERS_PROTECT) && m_targetProtect)
    {
        Unit* newTarget = FindAttacker((ATTACKERINFOTYPE)(AIT_VICTIMNOTSELF | AIT_HIGHESTTHREAT), m_targetProtect);
        if (newTarget && newTarget != m_targetCombat && !IsNeutralized(newTarget) && IsElite(newTarget) && GetFightFlg() &&( newTarget->GetEntry() != 14965 || newTarget->GetEntry() != 14750 || newTarget->GetEntry() != 11054))
        {
            forcedTarget = newTarget;
            m_targetType = TARGET_THREATEN;
            //if (m_mgr->m_confDebugWhisper)
    ...
    if (!m_targetCombat && !IsNeutralized(FindAttacker()))
    {
...
void PlayerbotAI::DoNextCombatManeuver()
{
    ...
    // bot attack stop
	if (m_bot->HasAura(24314) || m_bot->HasAura(9256))
	{
...
void PlayerbotAI::DoCombatMovement()
{
	...
	MaNGOS::NearestGameObjectEntryInObjectRangeCheck go_check3(*m_bot, 178164, 15.0f);
	MaNGOS::GameObjectLastSearcher<MaNGOS::NearestGameObjectEntryInObjectRangeCheck> searcher3(pGo, go_check3);
	Cell::VisitGridObjects(m_bot, searcher3, 15.0f);
	...
	if (
		m_bot->HasAura(21070) || m_bot->HasAura(17742) || m_bot->HasAura(23861) || m_bot->HasAura(24063) || m_bot->HasAura(19717) || m_bot->HasAura(20475) || m_bot->HasAura(23971) || m_bot->HasAura(23462) ||
		(m_targetCombat->GetEntry() == 14750 && m_targetCombat->GetHealthPercent() <= 55 && m_bot->GetCombatDistance(m_targetCombat) < 10.0f) ||
		(m_targetCombat->GetEntry() == 14517 && m_bot->GetCombatDistance(m_targetCombat) < 20.0f && !(m_combatStyle == COMBAT_MELEE)) ||
		...
		(m_targetCombat->GetEntry() == 15146 && m_bot->GetCombatDistance(m_targetCombat) < 5.0f && !(m_combatOrder & ORDERS_TANK)) ||
		(m_targetCombat->GetEntry() == 11382 && m_bot->GetCombatDistance(m_targetCombat) < 15.0f && !(m_combatOrder & ORDERS_TANK)) ||
		(m_targetCombat->GetEntry() == 12397 && m_bot->GetCombatDistance(m_targetCombat) < 15.0f && !(m_combatOrder & ORDERS_TANK)) ||
		(m_targetCombat->GetEntry() == 12056 && m_targetCombat->HasAura(19695) && m_bot->GetCombatDistance(m_targetCombat) < 20.0f) ||
		(m_targetCombat->GetEntry() == 11350 && m_targetCombat->HasAura(24018) && m_bot->GetCombatDistance(m_targetCombat) < 10.0f) ||
		(m_targetCombat->GetEntry() == 12264 && m_bot->GetCombatDistance(m_targetCombat) < 30.0f && !(m_combatStyle == COMBAT_RANGED)) 
		)
	{
		m_bot->AttackStop();
		m_bot->SetSelectionGuid(ObjectGuid());
	...
	if (m_targetCombat->HasAura(20619) && ((m_bot->getClass() == 8) || (m_bot->getClass() == 9)))
	{
		SetIgnoreUpdateTime(10);
		return;
	}
	
    //GetClassAI()->FleeFromAoEIfCan(21070, m_bot);
    ...
    if (m_combatStyle == COMBAT_MELEE
        ...
        && GetClassAI()->GetWaitUntil() < CurrentTime()) // Not waiting
    {
             ...
             && GetClassAI()->GetWaitUntil() < CurrentTime()) // Not waiting
    {
        ...
        {
            m_bot->GetMotionMaster()->Clear(false);
            m_bot->GetMotionMaster()->MoveChase(m_targetCombat,30.f);
        }
        else
            MovementClear();
    }
...
bool PlayerbotAI::CastNeutralize()
{
    ...
    creatureType = pCreature->GetCreatureInfo()->type;
...
void PlayerbotAI::DoLoot()
{
    ...
    if (m_bot->GetDistance(wo) < INTERACTION_DISTANCE)
    {
        ...
        if (c)  // creature
        {
            ...
            {
                // loot the creature
                WorldPacket packet = WorldPacket(CMSG_LOOT, 8);
        ...
        {
		//014 bot skin point adjust begin
            if (SkillValue < reqSkillValue)
        	{
				TellMaster("技能熟练度貌似不够，需要 %u,但我只有%u.管它呢，我尝试一下！",
					reqSkillValue, SkillValue);
				//skillFailed = true;
			}
			//014 bot skin point adjust end
            switch (skillId)
            {
                case SKILL_MINING:
                    if (CastSpell(MINING))
        ...
        {
            /*TellMaster(" 剥皮这事太血腥了我又不是波顿家族的丶");*/ //018 取消机器人不能剥皮时的对话反馈
            skillFailed = true;
        }
                ...
                if (!m_bot->GetQuestRewardStatus(questID))
                {
                    ...
                    if (pQuest->GetRewChoiceItemsCount() == 0)
                    {
                        if (m_bot->CanRewardQuest(pQuest, false))
                        {
                            ...
                            out << "任务完成主人: |cff808080|Hquest:" << questID << ':' << pQuest->GetQuestLevel() << "|h[" << questTitle << "]|h|r";
                        }
                        else
                    ...
                    {
                        out << "我该选择哪件东西主人 |cff808080|Hquest:" << questID << ':' << pQuest->GetQuestLevel()
                            << "|h[" << questTitle << "]|h|r? ";
...
// 增加识别耐久度
uint32 PlayerbotAI::EstDurabilityRepair(uint32 pos)
{
	Item* item = m_bot->GetItemByPos(pos);
        ...
        case ORDERS_PASSIVE: // 20(100000)
            {
                ...
                {
                    m_movementOrder = (MovementOrderType)(m_movementOrder & ~MOVEMENT_STAY);
                }
                SetMovementOrder(MOVEMENT_FOLLOW, GetMaster());
                return;
            }
...
{
    static UINT32 ulTimes = 0;//013 Bots Auto config
	ulTimes++;//013 Bots Auto config
    ...
    //013 Bots Auto config
	{
		uint8 t = m_ignoreAIUpdatesUntilTime - CurrentTime();
	...
	if(ulTimes%5 ==0)//013 Bots Auto config
	{
		_AutoSurvey(*GetMaster());
	}
    ...
    if (!m_bot->isAlive())
    {
        if (m_botState == BOTSTATE_DEAD && !(IsInCombat() && inRaidGroup))
        {
            // become ghost
            if (m_bot->GetCorpse())
            {
...
*/
        ...
        pSpellInfo->Effect[0] == SPELL_EFFECT_SKINNING)
    {
        if (m_lootCurrent)
        {
            ...
            WorldPacket packet = WorldPacket(CMSG_CAST_SPELL, 4+2+8);
            packet << spellId;
            packet << target_type;
...
bool PlayerbotAI::IsElite(Unit* pTarget) const
{
    ...
    if (Creature * pCreature = dynamic_cast<Creature*>(pTarget))
    {
...
bool PlayerbotAI::IsWorldBoss(Unit* pTarget) const
{
	if (!pTarget)
		return false;
...
static const uint32 uAurasIds[22] =
{
    118, 12824, 12825, 12826,   // polymorph
    28272, 28271, 28270,        // polymorph pig, turtle, Cow
...
void PlayerbotAI::UseItem(Item *item, uint16 targetFlag, ObjectGuid targetGUID)
{
    ...
    if (uint32 questid = item->GetProto()->StartQuest)
    {
        ...
        {
            m_bot->GetMotionMaster()->Clear(true);
            WorldPacket packet = WorldPacket(CMSG_QUESTGIVER_ACCEPT_QUEST, 8 + 4 + 4);
            packet << item_guid;
            packet << questid;
            packet << uint32(0);
            m_bot->GetSession()->QueuePacket(&packet); // queue the packet to get around race condition
    ...
    WorldPacket packet = WorldPacket(CMSG_USE_ITEM, 13);
    packet << bagIndex;
    packet << slot;
    packet << spell_count;
    packet << targetFlag;
...
static const uint32 uPriorizedHealingItemIds[13] =
{
    HEALTHSTONE_DISPLAYID, MAJOR_HEALING_POTION, WHIPPER_ROOT_TUBER, NIGHT_DRAGON_BREATH, GREATER_DREAMLESS_SLEEP_POTION,
...
bool PlayerbotAI::TradeItem(const Item& item, int8 slot)
{
    ...
    WorldPacket packet = WorldPacket(CMSG_SET_TRADE_ITEM, 3);
...
bool PlayerbotAI::TradeCopper(uint32 copper)
{
    if (copper > 0)
    {
        WorldPacket packet = WorldPacket(CMSG_SET_TRADE_GOLD, 4);
        packet << copper;
        m_bot->GetSession()->QueuePacket(&packet);
        return true;
    }
...
void PlayerbotAI::Repair(const uint32 itemid, Creature* rCreature)
{
    ...
    WorldPacket packet = WorldPacket(CMSG_REPAIR_ITEM, 8 + 8 + 1);
...
void PlayerbotAI::AddAuction(const uint32 itemid, Creature* aCreature)
{
    ...
    if (aItem)
    {
        ...
        WorldPacket packet = WorldPacket(CMSG_AUCTION_SELL_ITEM, 8 + 4 + 8 + 4 + 4 + 4 + 4);
...
void PlayerbotAI::HandleCommand(const std::string& text, Player& fromPlayer)
{
    ...
    if (!canObeyCommandFrom(fromPlayer))
    {
        ...
        {
            std::string msg = "主人不许我和陌生人说话有事找主人 ";
    ...
    else if (ExtractCommand("eat", input))
        _HandleCommandEat(input, fromPlayer, true);
    ...
    else
    {
        ...
        if (!itemIds.empty()) {
            ...
            WorldObject* const pNpc = (WorldObject *) m_bot->GetObjectByTypeMask(questRewarderGUID, TYPEMASK_CREATURE_OR_GAMEOBJECT);
            if (!pNpc)
                return;
        ...
        else
        {
			...
			std::string msg = "啥我听不懂你的命令主人打help试试.";
            SendWhisper(msg, fromPlayer);
            m_bot->HandleEmoteCommand(EMOTE_ONESHOT_TALK);
...
void PlayerbotAI::_HandleCommandOrders(std::string &text, Player &fromPlayer)
{
    ...
    //015 order bot change talent begin
	Player*ply = this->GetPlayer();

	//ply->SaveToDB();
	if (ExtractCommand("tf1", text))
	{
		ply->resetTalents(true);
		ply->_LoadTalent(1);
		return;
	}
	else if (ExtractCommand("tf2", text))
	{
		ply->resetTalents(true);
		ply->_LoadTalent(2);
		return;
	}
	else if (ExtractCommand("tf3", text))
	{
		ply->resetTalents(true);
		ply->_LoadTalent(3);
		return;
	}
    //015 order bot change talent end
    ...
    else if (ExtractCommand("combat", text, true))
    {
        ...
        if (ExtractCommand("protect", text) || ExtractCommand("assist", text))
        {
            ...
            if (text == "" && !targetGUID)
            {
                SendWhisper("|cffff0000让我协助攻击的命令必须先选择一个友方目标主人!", fromPlayer);
                return;
            }
                ...
                if (m_bot->IsWithinLOSInMap(thingToAttack))
                 {
                    ...
                    if (m_combatOrder & ORDERS_PASSIVE)
                    {
                        m_combatOrder = (CombatOrderType)(m_combatOrder & ~ORDERS_PASSIVE);
                    }
					if (m_movementOrder & MOVEMENT_STAY)
                    {
                        m_movementOrder = (MovementOrderType)(m_movementOrder & ~MOVEMENT_STAY);
...
void PlayerbotAI::_HandleCommandPull(std::string &text, Player &fromPlayer)
{
    ...
    if (m_bot->IsFriendlyTo(thingToAttack))
    {
        SendWhisper("我不能攻击我们的朋友主人.", fromPlayer);
        return;
    }
    ...
    if (!m_bot->IsWithinLOSInMap(thingToAttack))
    {
        SendWhisper("我看不见目标主人!", fromPlayer);
        return;
    }
...
void PlayerbotAI::_HandleCommandNeutralize(std::string &text, Player &fromPlayer)
{
    ...
    {
        SendWhisper("没有选择有效的目标.", fromPlayer);
        return;
    }

    if (m_bot->IsFriendlyTo(thingToNeutralize))
    {
        SendWhisper("你想把我们的朋友变羊?小心友尽.", fromPlayer);
        return;
    }

    if (!m_bot->IsWithinLOSInMap(thingToNeutralize))
    {
        SendWhisper("我看不见他!", fromPlayer);
        return;
    }

    if (IsNeutralized(thingToNeutralize))
    {
        SendWhisper("目标已被控制，我就不浪费蓝了.", fromPlayer);
        return;
    }
    ...
    if (!CastNeutralize())
    {
        SendWhisper("我控制不了它,换个人试试吧.", fromPlayer);
        return;
    }
}
...
void PlayerbotAI::_HandleCommandEat(std::string &text, Player &fromPlayer, bool bMana)
{
...
{   
    if (m_bot->getLevel() == 60)
    {
		...
		// equipped, backpack, bags itself
		for (int i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_ITEM_END; ++i)
			TotalDurability += EstDurabilityRepair(((INVENTORY_SLOT_BAG_0 << 8) | i));
		if (TotalDurability < 100)
		{
			TellMaster("主人,再不给我修装备人家可要裸体打怪了~~.");
			m_tasks.push_back(std::pair<enum TaskFlags, uint32>(REPAIR, 0));
			m_findNPC.push_back(UNIT_NPC_FLAG_REPAIR);
			return;
		}        
...
void PlayerbotAI::_HandleCommandPet(std::string &text, Player &fromPlayer)
{
    ...
    else if (ExtractCommand("toggle", text))
    {
        ...
        if (spellId != 0 && pet->HasSpell(spellId))
        {
            PetSpellMap::iterator itr = pet->m_petSpells.find(spellId);
            if (itr != pet->m_petSpells.end())
            {
...
//013 Bots Auto config
void PlayerbotAI::SetSurveyFlag()
{
	m_AutoSurvey = !m_AutoSurvey;
}
//013 Bots Auto config
void PlayerbotAI::SetAutoLoot()
{
	std::ostringstream detectout;
	m_AutolootFlg = !m_AutolootFlg;
	if(m_AutolootFlg)
		detectout << "看我摸尸体小红手  ";
	else
		detectout << "你行你来摸尸体吧  ";
	SendWhisper(detectout.str().c_str(), *(this->GetManager()->GetMaster()));


}
//013 Bots Auto config
void PlayerbotAI::SetAutoBuff()
{
	std::ostringstream detectout;
	m_AutoBuffFlg = !m_AutoBuffFlg;
	if (m_AutoBuffFlg)
		detectout << "加buff的事情交给我  ";
	else
		detectout << "你自己把buff加一下吧  ";
	SendWhisper(detectout.str().c_str(), *(this->GetManager()->GetMaster()));


}
void PlayerbotAI::SetAutoFight()
{
	std::ostringstream detectout;
	m_AutoFightFlg = !m_AutoFightFlg;
	if (m_AutoFightFlg)
		detectout << "控场，AOE交给我了  ";
	else
		detectout << "你自己控场AOE吧，我看看你的手能有多快! ";
	SendWhisper(detectout.str().c_str(), *(this->GetManager()->GetMaster()));
}
void PlayerbotAI::SetAutoDrag()
{
	std::ostringstream detectout;
	m_AutoDragFlg = !m_AutoDragFlg;
	if (m_AutoDragFlg)
		detectout << "把那瓶82年的大红给我起开  ";
	else
		detectout << "不喝不喝我们不喝! ";
	SendWhisper(detectout.str().c_str(), *(this->GetManager()->GetMaster()));
}
//013 Bots Auto config
void PlayerbotAI::SetBotsflag(uint32 flag)
{
	switch (flag)
	{
	    case 4:
...
//013 Bots Auto config
bool PlayerbotAI::CheckSkillReq(GameObject *go)
{
	// check skill or lock on object
	uint32 lockId = go->GetGOInfo()->GetLockId();
	LockEntry const *lockInfo = sLockStore.LookupEntry(lockId);
...
//013 Bots Auto config
bool PlayerbotAI::GoISQuestNeed(GameObject *go)
{
	std::ostringstream out;

	SetQuestNeedItems();
	QuestItemMap::iterator itr = m_QuestItemMap.find(go->GetGOInfo()->id);
    if (itr == m_QuestItemMap.end())
        return false;

	uint32 QuestItemId = itr->second;
	...
	for (BotNeedItem::iterator itr = m_needItemList.begin(); itr != m_needItemList.end(); ++itr)
	{
		...
		if (pItemProto)
		{
			if (QuestItemId == pItemProto->ItemId)
			{
				return true;
			}
#if 0
			std::string itemName = pItemProto->Name1;
			ItemLocalization(itemName, pItemProto->ItemId);
			//if ((go->GetGOInfo()->name) == itemName.c_str())
			//bool test1 = strncmp("test", "test", 4);
			//bool test = (strncmp(go->GetGOInfo()->name, itemName.c_str(), itemName.size()));

			if(!(strncmp(go->GetGOInfo()->name, itemName.c_str(), itemName.size())))
			{
				return true;
			}
#endif
...
//013 Bots Auto config
bool PlayerbotAI::IsMine(uint32 entry)
{
	uint32 aulMine[] = { 1731,1732,1733,1734,1735,2040,2047,2054,2055,2653,3763,3764,73939,\
		73940,73941,103711,103713,105569,123309,123310,123848,150079, 150080, 150081, \
		150082,165658,175404,176643,176645,177388,181108,181248,181249 };
	if (!m_bot->HasSkill(SKILL_MINING))
	{
		return false;
	}
	for (uint32 i = 0; i < (sizeof(aulMine) / sizeof(uint32)); i++)
	{
		if (aulMine[i] == entry)
			return true;
	}
...
//013 Bots Auto config
bool PlayerbotAI::IsHerb(uint32 entry)
{
	uint32 aulHerb[] = { 1617,3725,1618,3724,1619,3726,1620,3727,1621,3729,1622,3730,1623,1624,1628,2041,2042,2043,\
		2044,2045,2046,2866,142140,180165,142141,176642,142142,176636,180164,142143,183046,142144,\
		142145,176637,176583,176638,176584,176639,180168,176586,176640,180166,176587,176641,176588,176589};
	if (!m_bot->HasSkill(SKILL_HERBALISM))
	{
		return false;
	}
	for (uint32 i = 0; i < (sizeof(aulHerb)/sizeof(uint32)); i++)
	{
		if (aulHerb[i] == entry)
			return true;
	}
...
//013 Bots Auto config
void PlayerbotAI::_AutoSurvey( Player &fromPlayer)
{
	if (!m_AutoSurvey)
	{
		return;
	...
	if (result)
	{
		do
		{
			...
			if (!go->isSpawned())
				continue;
			//if (go->GetGOInfo()->name == "铜矿")
			//{

			//}
			if (GoISQuestNeed(go)||IsMine(entry)||IsHerb(entry))
			{
				if (IsMine(entry) || IsHerb(entry))
					if (!CheckSkillReq(go))
						continue;
				ObjectGuid lootCurrent = ObjectGuid(HIGHGUID_GAMEOBJECT, entry, guid);
				if(guid)
				{
					uint32 targetSize = m_lootTargets.size();

...
bool PlayerbotClassAI::EatDrinkBandage(bool bMana, unsigned char foodPercent, unsigned char drinkPercent, unsigned char bandagePercent)
{
    ...
    if (drinkItem || foodItem)
    {
        if (drinkItem)
        {
            ...
            m_ai->TellMaster("渴了，喝红牛~~.");
            m_ai->UseItem(drinkItem);
        }
        if (foodItem)
        {
			m_ai->SetIgnoreUpdateTime(3);
            m_ai->TellMaster("饿了，吃碗泡面吧~~.");
...
bool PlayerbotClassAI::NeedGroupBuff(uint32 groupBuffSpellId, uint32 singleBuffSpellId)
{
    ...
    if (m_bot->GetGroup())
    {
        ...
        for (Group::member_citerator itr = groupSlot.begin(); itr != groupSlot.end(); itr++)
        {
			...
			if (!pet)
                continue;
            if (pet && pet->isAlive() && !pet->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) && (pet->HasAura(groupBuffSpellId, EFFECT_INDEX_0) || pet->HasAura(singleBuffSpellId, EFFECT_INDEX_0)))
                numberOfGroupTargets++;
        }
	...
	if (Trinkets1)
	{
		...
		for (int32 Index1 = 0; Index1 < MAX_ITEM_PROTO_SPELLS; ++Index1)
		{
			if (pItemProto1->Spells[Index1].SpellTrigger != 0)
				continue;
			if (pItemProto1->Spells[Index1].SpellTrigger == 0 && (m_bot->IsSpellReady(pItemProto1->Spells[Index1].SpellId)))
			{
	...
	if (Trinkets2)
	{
		...
		for (int32 Index2 = 0; Index2 < MAX_ITEM_PROTO_SPELLS; ++Index2)
		{
			if (pItemProto2->Spells[Index2].SpellTrigger != 0)
				continue;
			if (pItemProto2->Spells[Index2].SpellTrigger == 0 && (m_bot->IsSpellReady(pItemProto2->Spells[Index2].SpellId)))
			{
